...
### [8.7 数据安全监控与审计](./8.7_数据安全监控与审计.md)
- 数据访问审计
- 异常行为检测（UEBA）
- 数据外泄检测
- 数据安全事件响应
...
### [8.9 实战案例](./8.9_数据安全实战案例.md)
- 全球电商数据分类体系建设
- 金融行业 DLP 实施案例
- 跨境数据传输合规架构
- 数据加密全生命周期案例

### [8.10 企业 PKI 与证书生命周期管理](./8.10_企业PKI与证书生命周期.md)
- 企业 CA 层级设计与密钥保管仪式（key ceremony）
- 自建 PKI、公共 CA、云托管私有 CA 的选型与隐性成本
- 证书清点：六种发现手段与各自盲区
- 自动化签发与续期（ACME、cert-manager、SPIFFE/SPIRE）与短周期证书
- 证书到期治理、紧急轮换、代码签名与 mTLS 运营负担
- 加密算法治理与后量子迁移准备（CBOM）
...
- 与 第1章（企业架构基础）：数据安全是安全架构三域之一，需嵌入企业架构体系
- 与 第2章（GRC）：数据安全治理与风险管理、合规要求紧密对齐
- 与 第3章（业务安全伙伴）：数据安全需与业务场景深度结合，通过 BISO 协作落地
- 与 第5章（云安全）：云数据保护是数据安全在云环境的延伸与实践
- 与 第9章（隐私合规）：数据安全与隐私保护协同，共同满足 GDPR、PIPL 等法规
- 与 第10章（信息保护）：信息保护团队负责 DLP 等技术控制的运营实施
- 与 第17章（AI for Cybersecurity）：AI 技术赋能数据分类、异常检测、自动化响应

---

## 导航

[← 上一章：第7章](../../第02部分_技术架构与基础设施安全/第07章_供应链安全/README.md) | [返回 Part 目录](../) | [下一章：第9章 →](../../第03部分_数据安全与隐私/第09章_隐私合规/)
