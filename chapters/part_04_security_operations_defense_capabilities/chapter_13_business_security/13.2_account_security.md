...
1. 过度依赖设备指纹：认为部署设备指纹后可以一劳永逸。实际上攻击方会在数天内找到绕过方法，需要持续对抗。

2. 忽视误报成本：设置过严的拦截规则导致大量正常用户无法注册，转化率下降带来的业务损失可能超过防护攻击带来的收益。

3. 缺乏降级方案：设备指纹 SDK 或服务出现故障时，如果没有降级方案会直接影响业务，需要设计降级逻辑（如切换到仅验证码模式）。
...
- 无法做到"实时完美拦截"，只能做到"快速响应并持续调优"
- 第一版策略几乎必然会过严或过松，需要二次迭代
- 团队需要 7×24 值班能力，大规模攻击时需要全员参与
...
1. 假设 Redis 永远可用：需要设计降级方案，Redis 故障时使用本地缓存或直接放行（记录风险日志）

2. 使用固定阈值：不同业务场景（大促/平时/攻击中）需要动态调整阈值

3. 忽视白名单机制：VIP 用户、企业用户、API 合作伙伴需要豁免

4. 单维度限制：IP 限制会误伤 NAT 用户，账号限制会误伤正常用户忘记密码多次尝试
...
- 新用户注册默认启用 MFA（但允许关闭）
- 高风险操作（大额提现、修改密码、修改绑定邮箱）强制要求 MFA 验证
- 定期安全提醒引导用户启用
- 激励措施补充（覆盖率提升有限）

这一组合方案的 MFA 覆盖率通常在双位数百分比，账号被盗率下降幅度也相应有限，但可以在用户体验与安全之间达到业务可接受的平衡点。
...
> **参考说明**：上述目标值基于电商/金融场景的风控模型运营经验。调整因素包括：
> 1. **业务场景**：社交平台对误拦率容忍度更高（可放宽至 < 1%），金融场景对拦截率要求更严格（≥ 90%）
> 2. **攻击强度**：大促/活动期间可临时收紧阈值，牺牲部分用户体验换取更高拦截率
> 3. **模型成熟度**：新模型上线初期 AUC 目标可调低至 ≥ 0.75，逐步优化后再提升目标
...
如果全部实时拦截，需要数十人 7×24 值班团队。实际部署通常配置数人至十余人团队，通过分级处理控制进审核量。

关键洞察：

- 钓鱼攻击发生在平台之外，无法事前预防，只能事后检测与快速响应
- "实时拦截"在现实中做不到，需要接受 5-30 分钟的响应时延
- 人工审核无法避免，需要在自动化与审核质量之间平衡
