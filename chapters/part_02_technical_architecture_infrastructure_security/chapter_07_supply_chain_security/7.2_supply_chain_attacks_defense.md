...
XZ Utils 是广泛使用的压缩工具库，被多数 Linux 发行版的 SSH 组件间接依赖。2024 年 3 月，安全研究人员发现 XZ Utils 5.6.0 和 5.6.1 版本中被植入了后门。

#### 攻击手法

攻击者使用社会工程手段长期渗透开源项目：首先以活跃贡献者身份获得项目维护权限（历时约两年）；随后在构建脚本中植入混淆的恶意代码，该代码仅在特定条件下（如 deb/rpm 打包构建）触发；最终后门劫持 SSH 认证流程，允许远程代码执行。

#### 技术特征

后门的隐蔽性设计包括：恶意代码分散在多个二进制测试文件中，通过构建脚本组装；仅在特定发行版的构建环境中激活，规避开发者本地测试；利用 IFUNC 机制劫持 OpenSSH 的 RSA 验证函数。

#### 防御教训

该事件对供应链安全的核心启示：代码审查流程需覆盖构建脚本和测试文件，而非仅关注源代码；应对关键基础设施组件实施维护者背景审查；二进制文件（包括测试数据）的变更应触发额外审查。
...
build_script_review:
  ...
  automated_checks:
    ...
    - build_time_network_access
```

**适用边界**：该防御措施适用于维护或依赖基础设施级开源组件（如压缩库、加密库、系统工具）的组织。

**常见误区**：一是信任长期贡献者——攻击者可通过持续贡献建立信任后实施攻击；二是仅审查 .c/.h 等源代码文件，忽略构建系统和测试数据。
...
**[← 上一节：7.1 软件供应链安全战略](./7.1_software_supply_chain_strategy.md)** | **[返回章节目录](./README.md)** | **[下一节：7.3 开源组件治理 →](./7.3_open_source_governance.md)**
